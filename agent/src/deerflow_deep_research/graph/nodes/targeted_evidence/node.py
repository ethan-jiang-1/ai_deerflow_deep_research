"""Real targeted_evidence node — dispatches critic work items from state.
...
from .subgraph import dispatch_critic


def build_real(dependencies: NodeBuildDependencies):
    """Build the real targeted_evidence node with critic dispatch."""

    async def run(state: dict[str, Any]) -> dict[str, Any]:
        work_items = state.get("critic_work_items") or ()
        if not work_items:
            return {**node_update("targeted_evidence"), "route": "next"}
        ...
        for entry in results:
            ...
            if isinstance(result, SourceDiagnosticResult):
                ...
                materialize_source_diagnostic(result, node_attempt_id, Path(workspace_root), allowed_source_ids=allowed)
            elif isinstance(result, ClaimVerifierResult):
                allowed = set()
                for item in work_items:
                    if item.get("type") == "claim_verifier":
                        allowed.update(item.get("evidence_refs", ()))
                materialize_claim_verifier(result, node_attempt_id, Path(workspace_root), allowed_source_ids=allowed)
