...
def _hard_invariants(
    ...
    return {
        ...
        "lifecycle_trace_complete": tuple(outcome.lifecycle_trace) == scenario.expected_trace,
    ...
    *,
    ...
    if not isinstance(started, Command):
        return ReleaseAttempt(
            None,
            f"start:{started.get('code', 'invalid')}",
            time.monotonic() - started_at,
            tracker.input_tokens or None,
            tracker.output_tokens or None,
            web.calls,
            tuple(tracker.response_shapes[-16:]),
        )
    ...
    if not isinstance(first_resume, Command):
        return ReleaseAttempt(
            None,
            f"first_resume:{first_resume.get('code', 'invalid')}",
            time.monotonic() - started_at,
            tracker.input_tokens or None,
            tracker.output_tokens or None,
            web.calls,
    ...
    accepted = tuple(f"ref:{record.record_hash}" for record in records)
