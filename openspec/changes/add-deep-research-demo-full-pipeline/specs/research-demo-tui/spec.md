...
The agent project SHALL provide a Textual demo entry that invokes the existing reflected-tool lifecycle dispatch with a retained process-local memory host and test-owned trusted runtime envelope, using the all-real implementation mode (`ALL_REAL_MODES`) through `ResearchGraphRecipe.create()` with `RuntimeNodeAgentBridge`. It SHALL traverse a visible request, HITL1 text response, HITL2 advertised decision, and terminal or cancelled result. The demo SHALL validate that `ANTHROPIC_API_KEY` is set at startup and exit with a clear error if unavailable. The demo SHALL display pipeline phase progress in the UI driven by the `execution_trace` field from each `DeepResearchControlResult`. It SHALL NOT introduce a second graph, checkpoint schema, fixture authority, or research artifact. (`RED-001`)
...
- **WHEN** the demo target runs in the locked agent environment without `ANTHROPIC_API_KEY` set
- **THEN** the Textual application exits with a clear error message before building the graph
