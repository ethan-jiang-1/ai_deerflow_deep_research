...
required for its internal `Send` fan-out/fan-in. Neither exception MAY import graph
implementation modules, agents, runtime, or sibling nodes. Runtime SHALL be the only layer allowed to bind
...
- **THEN** every import resolves within the allowed layer direction and the subgraph does not gain access to graph implementation, runtime, agents, or sibling nodes

#### Scenario: Reverse dependency fails closed
- **WHEN** a fixture makes domain code import runtime code, an ordinary node module import LangGraph beyond the exact HITL `interrupt` exception, a node import agents/runtime/graph implementation, or one node import another node
...
Reusable cross-node non-top-level subflows SHALL remain under `graph/components/`.
...
- **WHEN** a fixture supplies the required node files, a valid `NODE_SPEC`, and an optional phase-local subgraph using only its permitted imports
...
- **WHEN** a node package omits its fake, exports internal callables directly, uses an unstable logical name, or exposes a phase-local worker as a top-level node
- **THEN** registry validation fails before graph compilation

#### Scenario: Node-to-registry cycle is refused
- **WHEN** a node package imports `graph.registry`, another graph implementation module, or a sibling node to construct its spec or subgraph
- **THEN** the import contract fails and directs the node to the pure domain NodeSpec or package-local subgraph contract
