...
- No project-owned live launch WRAPPER (`serve.sh` stop/start orchestration and `agent/Makefile` dev/prod/daemon targets), no in-container prelaunch doctor GATE before uvicorn, and no live dev/production/Docker launch smoke — these require a provisioned deployment environment and are deferred to a follow-up deployment change (`_backlog/todos/deferred_deep-research-00-launcher-and-docker.md`). Change 00 delivers only the loadable-source mechanism (`prepare.py`, the committed Docker override, the startup-fingerprint contract) and its contract tests.
- No real Postgres durability profile or Postgres test Compose service; file-backed SQLite is the change-00 durable store, and Postgres is deferred (`_backlog/todos/deferred_deep-research-00-postgres-profile.md`).
