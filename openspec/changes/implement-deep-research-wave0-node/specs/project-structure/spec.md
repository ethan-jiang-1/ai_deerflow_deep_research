...
Real topic planning SHALL add only canonical downstream production paths owned by that
...
worker prompt) and `agent/src/deerflow_deep_research/graph/nodes/wave0/gate.py` (the
real Wave0 source-floor gate rules). The real `wave0.source-intake` result-contract
model SHALL live inside the existing wave0 package. These paths SHALL be registered
...
- **WHEN** the folder contract inspects this change after implementation
- **THEN** the new `wave0/prompts.py` and `wave0/gate.py` paths appear under the canonical downstream package, wave0 imports only `domain`/`engine`, and no Deep Research source appears under `backend/` or `frontend/`
