...
The shared scripted model fixture follows DeerFlow's proven `FakeToolCallingModel` pattern but retains the local `ScriptedChatModel` name and project-owned fixture module. Behavioral model/tool tests construct the real project agent or runtime bridge and preprogram only the external model turns and tool results. The fixture records calls and bindings, fails immediately when the script is exhausted, and is checked for unconsumed turns at case completion so a path cannot silently stop early. Patching `create_agent`, a bridge builder, or another assembly factory remains appropriate for constructor arguments, middleware order, tool exposure, and other wiring assertions; such a patched-assembly test cannot claim agent-loop or `SCRIPTED_REAL_WORKFLOW` behavior.

Replay is an escalation, not the default synonym for scripted turns. A case uses a short in-memory script when ordered model/tool turns and production authority outcomes are sufficient. A persisted replay is justified only when a concrete provider or multi-caller trace cannot be represented faithfully that way, for example caller-sensitive interleaving or stable event-shape drift. Any replay key includes caller identity, normalizes only declared volatile UUID/time/path fields, fails on misses and unconsumed turns, and asserts stable shape/order rather than volatile values or raw prose. This change therefore does not add a generalized `ReplayChatModel` framework unless a migrated case records that specific insufficiency and returns to review of the smallest fixture needed.

Reusable fixtures establish a hermetic case boundary: isolate the effective home/config paths and contained filesystem, inject explicit local runtime configuration, reset or restore mutable sandbox-provider and project/DeerFlow singleton state, and prove cleanup even after failure. The existing public-network denial remains a separate suite-wide guard; it does not substitute for state cleanup. Shared fixture consolidation may remove the local public-entry replay model duplication only when its behavior remains explicit and its focused selector continues to prove the same entry contract.
...
Governance is introduced incrementally so every apply task can finish green. Early contract tasks use synthetic valid/invalid registries to prove failure behavior without making the partially migrated repository an expected CI failure. Real-catalog enforcement is enabled only at its closure task: current model/tool-node workflow coverage in task 1.4, all ten scenario families in tasks 3.11-3.12, provider-discovery closure in task 4.8, and cross-lane requirement policy in task 7.1. Baseline probes record current failures such as the empty workflow collection, but no checked task leaves a deliberately failing repository gate for a later group to repair.

Every new or materially changed static/runtime enforcement gate includes a meta-test that intentionally presents one minimal violation and proves the gate reports it. This applies to selector partitioning, evidence-claim and scenario joins, node discovery, provider provenance, requirement policy, sensitivity scans, report/attestation validation, and deadline enforcement; the existing blocking-I/O detector keeps its own detector smoke test. A passing repository with no negative probe is not sufficient evidence that a gate still detects its target failure.

The durable policy is split by authority rather than copied verbatim into three places:

- the `evaluation-hardening` spec owns the observable and mechanically verifiable test-evidence requirements delivered by this change;
- `openspec/governance/test-evidence-policy.md` owns cross-change planning/review policy: deterministic-first evidence, lowest responsible seam, real loop with fake external dependency, scripted-versus-replay-versus-live/release routing, stable-shape assertions, fixture isolation, gate self-tests, and prohibition on test-count/line-coverage proxies;
...
The root OpenSpec governance scripts remain read-only and Python-standard-library-only. This change does not make them invoke pytest, import agent dependencies, or maintain another exhaustive selector catalog. `openspec/governance/README.md` will list the policy as a normative fifth governance pillar while distinguishing it from the executable agent-owned checker.
...
| `EVH-005` | disjoint lanes, six bounded live cases, versioned reports, and singular persisted release proof (tasks 1.5-1.7, 5.5, 6, and 7.2) |
| `EVH-006` | central exact-selector claims, migrated incident inventories, and live/release discovery provenance (tasks 1.2-1.3, 4, and 7.2) |
| `EVH-007` | canonical seams/authenticity, governed scenario cases, observations, and valid seeded preconditions (tasks 1.2, 2, 3.11, and 6.1) |
| `EVH-008` | audited real-node/workflow coverage plus first-wave and gap-closing workflow replays (tasks 1.3-1.4 and 3) |
| `EVH-009` | five disjoint focused selections, deterministic partition, collected-claim governance, and cross-lane policy (tasks 1.4-1.7, 2.6, 3.11, 6, and 7.1) |
...
- **[Risk] Test guidance diverges across spec, governance, config, and agent docs.** -> Assign one authority to each layer, keep `openspec/config.yaml` and `agent/AGENTS.md` concise, and link to the governance policy instead of copying its full vocabulary or catalogs.

## Migration Plan

1. Record the empty-workflow/placeholder-metric baseline and add synthetic red/green governance fixtures for inert families, split vocabularies, overlap, and invalid metric authority without leaving the repository gate red.
...
7. Commit the minimized release attestation, reconcile stale evidence documents, add the durable test-evidence policy and concise OpenSpec/agent pointers, tighten executable agent governance, run the complete deterministic/specialized/live validation matrix, and avoid rerunning full-real unless the explicit rerun policy is triggered.
...
None after review round 8. Scope expansion that changes production behavior, schemas, or interfaces requires returning to proposal/design review before implementation.
