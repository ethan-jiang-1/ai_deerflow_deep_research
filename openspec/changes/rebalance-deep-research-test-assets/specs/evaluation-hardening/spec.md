...
- **THEN** the real agent or runtime bridge SHALL consume them, every scripted turn SHALL be consumed exactly once, and premature exhaustion, unused turns, unexpected tool exposure, or unexpected calls SHALL fail the case
...
- **WHEN** a deterministic workflow case changes effective config/home paths, sandbox-provider state, singleton caches, or contained filesystem data and then succeeds or raises
- **THEN** the next case SHALL observe its own explicit local configuration and clean filesystem with no prior provider, singleton, path, checkpoint, ledger, or artifact state
...
#### Scenario: Enforcement gate proves its detector works
- **WHEN** selector, claim, scenario, node-discovery, provenance, requirement, sensitivity, report, attestation, or deadline enforcement is added or materially changed
- **THEN** a focused meta-test SHALL introduce one minimal target violation and assert the gate fails for that reason rather than merely asserting that the valid repository passes
